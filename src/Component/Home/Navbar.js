...
const Navbar = () => {
  ...
  return (
    <div className={`sticky top-0 w-full z-50 transition-all duration-300 ${scrolled ? 'shadow-xl' : ''}`}>
...
<nav className={`bg-gradient-to-r from-gray-900 to-black text-white transition-all duration-300 ${scrolled ? 'py-1' : 'py-2'}`}>
  <div className="container mx-auto flex justify-between items-center px-6">
    ...
    <Link to="/" className="flex items-center space-x-2 group">
      <img 
        ...
        className={`transition-all duration-300 ${scrolled ? 'h-14 w-24' : 'h-16 w-28'} group-hover:opacity-90`} 
      />
      <div className="flex flex-col">
        <span className="font-bold text-2xl bg-clip-text text-transparent bg-gradient-to-r from-blue-100 to-white group-hover:from-blue-200 group-hover:to-white transition-all duration-200">Tesod</span>
        <span className="font-light text-base text-blue-200 group-hover:text-blue-100 transition-colors duration-200">Technology</span>
      </div>
    </Link>
            ...
            {navItems.map(({ name, path }) => (
              <Link 
                ...
                className={`relative font-medium px-1 py-2 hover:text-blue-300 transition-colors duration-200
                  ${location.pathname === path ? 'text-blue-300' : 'text-gray-100'}
