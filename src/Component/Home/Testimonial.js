  ...
  return (
    <div className="flex items-center">
      {[...Array(fullStars)].map((_, i) => (
        <Star key={`star-${i}`} className="w-5 h-5 text-amber-500 fill-amber-500" />
      ))}
      
      {hasHalfStar && <StarHalf className="w-5 h-5 text-amber-500 fill-amber-500" />}
...
const TestimonialsPage = () => {
  ...
  const goToPrev = () => {
    ...
    setIsAnimating(true);
    setActiveIndex((prev) => (prev === 0 ? testimonialData.length - itemsToShow : prev - 1));
  ...
  const goToNext = () => {
    ...
    setIsAnimating(true);
    setActiveIndex((prev) => (prev === testimonialData.length - itemsToShow ? 0 : prev + 1));
    setTimeout(() => setIsAnimating(false), 500);
  };
    ...
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-gray-100">
      ...
      <header className="bg-gradient-to-r from-blue-600 to-indigo-700 py-16 px-4">
        <div className="max-w-6xl mx-auto text-center">
          <h1 className="text-4xl font-bold mb-4 text-white">What Our Clients Say</h1>
      ...
      <div className="max-w-6xl mx-auto px-4 py-16 relative">
          ...
          >
            {testimonialData.map((testimonial) => (
              <div 
                key={testimonial.id} 
                className="px-3"
                style={{ width: `${100 / testimonialData.length * itemsToShow}%` }}
              >
        ...
        <div className="flex justify-center mt-8 space-x-2">
          {[...Array(testimonialData.length - itemsToShow + 1)].map((_, index) => (
            <button
              key={index}
